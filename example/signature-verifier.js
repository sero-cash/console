 ...
 * a. Signing a message by an Serocash user
...
function initializeSerocashConnection(){
   if(seroWeb3!=null && seroWeb3.isConnected()==true)  {
    return true;
  }
  
  seroWeb3 = new Web3(new Web3.providers.HttpProvider(seroURL));
  
  if(seroWeb3.isConnected()==true){
      if(defaultAc==''){
        defaultAc=seroWeb3.sero.accounts[1];
      }
      return true;
  }
...
function unlockAccount(acAddress){
  if(acAddress!=undefined && acAddress!=null){
    var state=seroWeb3.personal.unlockAccount(defaultAc, defaultAcPWD, 100);
    return state;
  }
...
function initializeContract(){
    initializeSerocashConnection();
    if(seroWeb3.isConnected()==false){
        return;
    }  
    var abi = JSON.parse(strAbi);
    var contract = seroWeb3.sero.contract(abi);
...
function signMessage(message){
    ...
    const sig = seroWeb3.sero.sign(defaultAc, '0x' + msg.toString('hex'));
...
function verifySignedByAc(message, sig){
    ...
    // Unfortunately Gero client adds this line to the message as a prefix while signing
    // So while finding who signed it we need to prefix this part 
    const prefix = new Buffer("\x19Serocash Signed Message:\n");
    const msg = new Buffer(message);
    const prefixedMsg = seroWeb3.sha3(
...
function splitSig(sig) {
  return {
    v: seroWeb3.toDecimal('0x' + sig.slice(130, 132)),
    r: sig.slice(0, 66),
    s: sig.slice(66, 130)
  }
...
function execute(){
    ...
    console.log("\ta. Serocash URL");
    console.log("\tb. Serocash Account Address");
    console.log("\tc. Serocash Account Passphrase");
    console.log("\td. Signature Contract Address");
    console.log("\te. Message for signing");
    console.log("**********************************************************************");

    if(seroURL==''){
        console.log("Error: Serocash URL is not specified");
        return;
    }
    if(defaultAc==''){
    ...
    console.log("\ta. Serocash URL                  :",seroURL);
    console.log("\tb. Serocash Account Address      :",defaultAc);
    console.log("\tc. Serocash Account Passphrase   :",defaultAcPWD);
...
setSerocashURL('http://localhost:8545');

// Value 5- If required please update with a Serocash URL
